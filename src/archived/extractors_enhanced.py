...
def extract_admin_fee(text: str) -> str:
    ...
    # To to catch "GPO Administrative Fee Percentage ... 4.00%"
    gpo_pattern = r'(?:GPO|Administrative)\s+(?:Administrative\s+)?Fee\s+Percentage[^%\d]*(\d+\.?\d*)%'
    match = re.search(gpo_pattern, text, re.IGNORECASE)
    if match:
        return f"{match.group(1)}% WAC"
    
    # Simpler pattern for just finding percentage near "fee"
    simple_pattern = r'(?:fee|fees)[^%\d]{0,100}(\d+\.?\d*)%'
    match = re.search(simple_pattern, text, re.IGNORECASE)
    if match:
        return f"{match.group(1)}%"
    
    # Look for all fee percentages
